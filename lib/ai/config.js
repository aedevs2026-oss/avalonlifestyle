...
import { isHuggingfaceConfigured } from "@/lib/ai/huggingface";
...
function defaultChatProvider() {
  if (process.env.AI_CHAT_PROVIDER) return process.env.AI_CHAT_PROVIDER.toLowerCase();
...
export function getChatConfig() {
  const provider = defaultChatProvider();
  return {
    provider,
    model:
...
export function isChatLlmConfigured() {
  ...
  if (c.provider === "grok") return isGrokConfigured();
  if (c.provider === "huggingface" || c.provider === "hf") return isHuggingfaceConfigured();
  if (c.provider === "gemini") return isGeminiConfigured();
  if (c.provider === "openai") return Boolean(c.openaiApiKey);
  if (c.provider === "ollama") return Boolean(c.ollamaBaseUrl);
  return false;
