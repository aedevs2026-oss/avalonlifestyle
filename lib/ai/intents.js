import { detectLanguageFromText } from "@/lib/ai/language";
import { extractCityFromQuestion } from "@/lib/ai/dealerSearch";
...
const BUDGET_RE =
  /(?:below|under|less than|within)\s*(?:₹|rs\.?|inr)?\s*([\d,]+(?:\.\d+)?)\s*(k|lakh|lac)?\b|(?:₹|rs\.?|inr)\s*([\d,]+(?:\.\d+)?)\s*(k|lakh|lac)?|([\d,]+)\s*(k|lakh|lac)\b/i;
...
export function detectIntent(question) {
  ...
  if (
    /\b(human|agent|representative|callback|speak to someone|talk to someone)\b/.test(q) ||
    /\b(track|order status|my order)\b/.test(q)
  ) {
  ...
  const cityHint = filters.city || extractCityFromQuestion(question);
  const nearPlace =
    /\bnear\b/.test(q) &&
    cityHint &&
    (/\bavalon\b/.test(q) ||
      /\b(showroom|showrooms|store|stores|shop|outlet|buy)\b/.test(q) ||
      (!/\b(mattress|mattresses|price|warranty|compare|versus|vs|foam|firm|king|prince)\b/.test(q) &&
        String(question || "").trim().split(/\s+/).length <= 6));

  if (
    /\b(dealers?|showrooms?|stores?|near me|find a dealer|dealer locator|where (can i|to) buy)\b/.test(q) ||
    /\bnear\s+dealers?\b/.test(q) ||
    nearPlace
  ) {
    if (cityHint && !filters.city) filters.city = cityHint;
  ...
  if (
    /\b(recommend|suggest|best|suitable|side sleeper|back sleeper|couple|venum|venum|nalla|irukkum|show me|kaatu|kulla)\b/.test(
      q,
    ) ||
    filters.budget_max ||
    filters.firmness
...
export function extractFilters(question) {
  ...
  if (budget) {
    let num = Number(String(budget[1] || budget[3] || budget[5]).replace(/,/g, ""));
    const unit = (budget[2] || budget[4] || budget[6] || "").toLowerCase();
    if (unit === "k") num *= 1000;
...
function detectLanguage(q) {
  return detectLanguageFromText(q);
}
