...
import { formToPayload } from "@/lib/admin/productForm";
import { disableProductChunks } from "@/lib/ai/pipeline";
...
export async function upsertCategory(form) {
  ...
  const categoryId = normalizeUuid(form.id);
  const row = {
    slug,
    name,
  ...
  const { error } = categoryId
    ? await db.from("categories").update(row).eq("id", categoryId)
    : await db.from("categories").insert(row);

  if (error) throw new Error(error.message || "Could not save category");
...
export async function deleteCategory(id) {
  ...
  if (error) throw new Error(error.message || "Could not delete category");
...
export async function upsertProduct(form) {
  ...
  const productId = normalizeUuid(form.id);
  let existingPayload = {};
  if (productId) {
    const { data: existing } = await db.from("products").select("payload").eq("id", productId).maybeSingle();
  ...
  if (form.payloadJson) {
    try {
      payload = { ...existingPayload, ...JSON.parse(form.payloadJson) };
    } catch {
      throw new Error("Invalid product JSON payload");
    }
  } else {
    payload = formToPayload(form, existingPayload);
  }
  ...
  const row = {
    ...
    category_id: normalizeUuid(form.category_id),
    payload,
  ...
  let error;
  if (productId) {
    ({ error } = await db.from("products").update(row).eq("id", productId));
  } else {
    ({ error } = await db.from("products").insert(row));
  }

  if (error) {
    console.error("[upsertProduct]", error);
    throw new Error(error.message || "Could not save product");
  }
  ...
  const { data: saved } = await db.from("products").select("id, is_published").eq("slug", slug).maybeSingle();
  if (saved?.id && !saved.is_published) {
    try {
      await disableProductChunks(saved.id);
    } catch (e) {
      console.error("[ai disable product chunks]", e);
    }
  }
  // Published product vectors: re-index from Admin → AI Knowledge (avoids slow Gemini calls blocking saves).
...
export async function deleteProduct(id) {
  const db = await adminDb();
  const productId = normalizeUuid(id);
  if (!productId) throw new Error("Invalid product id");

  const { data: row } = await db.from("products").select("slug").eq("id", productId).maybeSingle();
...
export async function upsertStory(form) {
  ...
  const storyId = normalizeUuid(form.id);
  const slug =
  ...
  const row = {
    ...
    category_label: String(form.category_label || "").trim() || null,
    sort_order: Number(form.sort_order) || 0,
  ...
  const { error } = storyId
    ? await db.from("stories").update(row).eq("id", storyId)
    : await db.from("stories").insert(row);

  if (error) throw new Error(error.message || "Could not save story");
...
export async function upsertBrochure(form) {
  ...
  const brochureId = normalizeUuid(form.id);
  const fileUrl = String(form.file_url || "").trim() || "#";
  ...
  const row = {
    ...
    category_label: String(form.category_label || "").trim() || null,
    version: String(form.version || "").trim() || null,
    sort_order: Number(form.sort_order) || 0,
    is_published: form.is_published !== false,
    updated_at: new Date().toISOString(),
  };

  const { error } = brochureId
    ? await db.from("brochures").update(row).eq("id", brochureId)
    : await db.from("brochures").insert(row);

  if (error) throw new Error(error.message || "Could not save brochure");
...
export async function deleteBrochure(id) {
  const db = await adminDb();
  const brochureId = normalizeUuid(id);
  if (!brochureId) throw new Error("Invalid brochure id");

  const { error } = await db.from("brochures").delete().eq("id", brochureId);
  if (error) throw new Error(error.message || "Could not delete brochure");
...
export async function upsertDealer(form) {
  ...
  const dealerId = normalizeUuid(form.id);
  const row = {
  ...
  const { error } = dealerId
    ? await db.from("dealers").update(row).eq("id", dealerId)
    : await db.from("dealers").insert(row);

  if (error) throw new Error(error.message || "Could not save dealer");
...
export async function deleteDealer(id) {
  const db = await adminDb();
  const dealerId = normalizeUuid(id);
  if (!dealerId) throw new Error("Invalid dealer id");

  const { error } = await db.from("dealers").delete().eq("id", dealerId);
  if (error) throw new Error(error.message || "Could not delete dealer");
...
export async function updateContactSubmissionStatus({ id, status, admin_notes }) {
  const db = await adminDb();
  const submissionId = normalizeUuid(id);
  ...
  const { error } = await db
    ...
    .eq("id", submissionId);
  if (error) throw new Error(error.message || "Could not update enquiry");
  revalidateAdmin();
  return { ok: true };
}
...
export async function updateDealerApplicationStatus({ id, status, admin_notes }) {
  const db = await adminDb();
  const applicationId = normalizeUuid(id);
...
export async function convertDealerApplicationToDealer(applicationId) {
  const db = await adminDb();
  const appId = normalizeUuid(applicationId);
  if (!appId) throw new Error("Invalid application id");
  ...
  const { data: dealer, error: insertError } = await db
    ...
    .select("id")
    .single();
  if (insertError) throw new Error(insertError.message || "Could not create dealer");
