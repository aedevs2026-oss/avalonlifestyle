...
import { disableProductChunks, indexProductById } from "@/lib/ai/pipeline";
...
export async function upsertProduct(form) {
  ...
  if (saved?.id) {
    try {
      if (saved.is_published) await indexProductById(saved.id);
      else await disableProductChunks(saved.id);
    } catch (e) {
      console.error("[ai index product]", e);
    }
  }
